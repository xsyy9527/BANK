...
Slideshow.prototype.initUI = function(){
	...
	for(var i=0;i<this.imgs.length;i++){
		let str = "<img src='"+this.imgs[i]+"' style='width:"+this.width+";height:"+this.height+";position:absolute;z-index:"+(this.imgs.length-i)+";display:none'/>";
	...
	//2、所有按钮的创建(定位按钮的位置）
	//2、所有按钮的创建(定位按钮的位置）
	let ulliStr = "<ul style='position:absolute;right:343px;bottom:300px;z-index:9;display:flex;justify-content:space-between'>";
	
	for(let i=0;i<this.imgs.length;i++){
		ulliStr +="<li ord='"+(i+1)+"' style='list-style:none;float:left;margin:10px;width:"+this.btnWidth+"px;height:"+this.btnHeight+"px;border-radius:5px;text-align:center;background-color:"+this.btnColor+";'>"+(i+1)+"</li>"
	}
	ulliStr +="</ul>";
